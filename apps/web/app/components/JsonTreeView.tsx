...
const JsonTreeNode: React.FC<JsonTreeViewProps> = ({
  ...
  const renderIcon = () => {
    if (isArray)
      return (
        <span className='text-zinc-500 font-bold px-1 text-[10px]'>[]</span>
      )
    if (isObject)
      return (
        <span className='text-zinc-500 font-bold px-1 text-[10px]'>{'{}'}</span>
    ...
    <div className='font-mono text-xs leading-5 select-none whitespace-nowrap'>
      ...
      >
        ...
        >
          {isComplex &&
            !isEmpty &&
            (expanded ? (
              <div className='border border-zinc-400 dark:border-zinc-600 bg-white dark:bg-zinc-800 w-[9px] h-[9px] flex items-center justify-center rounded-[1px] shadow-sm'>
                <span className='text-[7px] leading-none mb-px font-bold text-zinc-600 dark:text-zinc-300'>
                  -
                </span>
              </div>
            ) : (
        ...
        {/* Icon based on type */}
        <div className='mr-1 flex items-center opacity-70'>{renderIcon()}</div>
      ...
      {isComplex && expanded && !isEmpty && (
        <div className='ml-[5px] border-l border-dotted border-zinc-300 dark:border-zinc-700 pl-3'>
          {isArray
            ? data.slice(0, visibleLimit).map((item: any, index: number) => {
          ...
          {(isArray ? data.length : Object.keys(data).length) >
            ...
            <button
              ...
              className='mt-1 text-xs text-emerald-600 hover:text-emerald-500 font-medium pb-1 cursor-pointer block'
            >
              Show More...
            </button>
          )}
